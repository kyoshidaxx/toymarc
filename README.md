...
**注意**: 
- 初回起動時に自動的に `.env` ファイルの作成、アプリケーションキーの生成、フロントエンドビルドが実行されます
- ビルド時に自動的に依存関係のインストール、マイグレーション、シードが実行されます

### 手動セットアップ（詳細）

**環境設定をカスタマイズしたい場合のみ、以下の手順を実行してください：**
...
**注意**: 
- 初回起動時に自動的に `.env` ファイルの作成とアプリケーションキーの生成が実行されます
